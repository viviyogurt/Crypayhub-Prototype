<template>
    <div>
        <p style="color: var(--secondary); font-size: small" v-if="subscriptionServices.length===0">No Service For
            Subscription</p>
        <div class="serviceCardsContainer">
            <div v-for="(item, index) in subscriptionServices" :key="index">
                <div class="card">
                    <div class="box">
                        <div class="content">
                            <h2>{{index+1}}</h2>
                            ...
                            <b-button block variant="primary" class="my-3" @click="hardCodeListener()">Collect Fee</b-button>
                            <b-button block variant="danger" class="my-3">Delete</b-button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        mounted(){
            this.$root.$on('updateSubscriptionServices',() =>{
        ...
        methods: {
            getSubscriptionService() {
                let params = {merchant: this.senderAccount};
                reqRealEnd(
                    "get",
                    backendConfig.backendQueryURL,
                    backendConfig.backendAPI.getSubscriptionServices + "?" + Qs.stringify(params),
                    null
                ).then((res) => {
                    console.log(res);
                    if (res.data.data.length > 0) {
                        this.subscriptionServices = res.data.data;
                    } else {
            ...
            async hardCodeListener(){
                ...
                this.$root.$emit('successAlert', true, "Success to collect the service fee. 0 customers have been charged for membership extension!")
            }
        }
    }
</script>
